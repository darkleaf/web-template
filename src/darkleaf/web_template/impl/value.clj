...
(defn write-value-map [this w ctx block inverted-block]
  (if (seq this)
    (p/render block w (ctx/push ctx this))
    (p/render inverted-block w ctx)))

(defn write-value-seqable [this w ctx block inverted-block]
  (if (seq this)
    (doseq [item this]
      (p/render block w (ctx/push ctx item)))
    (p/render inverted-block w ctx)))

(defn write-value-default [this w ctx block inverted-block]
  (p/render block w (ctx/push ctx this)))
